...
public class LoginListener implements Listener {
	...
	public void onPlayerLogin(PlayerLoginEvent event) {
		...
		if (!storage.isPlayerReal(event.getPlayer())) {
			event.disallow(Result.KICK_OTHER, ChatColor.DARK_RED + DatDuplicateRemover.getInstance().getConfig().getString("locale.InvalidCase") + ChatColor.RESET + "\n" + DatDuplicateRemover.getInstance().getConfig().getString("locale.SpellNameCorrect") + ": " + ChatColor.GOLD + storage.getRealPlayerValidName(event.getPlayer()));
		}
	}
